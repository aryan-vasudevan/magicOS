...
class magicOSApp:
    ...
    def create_layout(self):
        ...
        # Placeholder for dynamic section
    ...
    def update_action_panel(self):
        ...
        if self.action_type.get() == "keybind":
            ttk.Button(self.action_container, text="Start Recording Keybind", command=self.record_keybind).pack()
        else:
    ...
    def record_keybind(self):
        messagebox.showinfo("Recording", "Start recording keybind... (implement hook here)")

    def get_installed_apps(self):
        app_names = sorted([app.name for app in winapps.list_installed()])
        return app_names if app_names else ["No apps found"]
